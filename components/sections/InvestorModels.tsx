...
const cardTheme: Record<InvestorModel["id"], CardTheme> = {
  compra: {
    card: "border border-ink/15 bg-white",
    badge: "bg-ink/10 text-ink/70",
    claim: "text-ink",
    copy: "text-muted",
    benefitIcon: "text-primary",
    benefitText: "text-ink/85",
    idealFor: "text-muted",
    cta: "bg-ink text-paper hover:bg-ink/85",
  },
  fleet: {
    card: "border border-primary/20 bg-white",
    ...
    benefitText: "text-ink/85",
    idealFor: "text-muted",
    cta: "bg-primary text-paper hover:bg-primary-bright",
  },
  city: {
    ...
    claim: "text-volt",
    copy: "text-paper/80",
    benefitIcon: "text-volt",
    benefitText: "text-paper/90",
    idealFor: "text-paper/60",
    cta: "bg-volt text-ink hover:shadow-lift",
  },
};

function ctaEventName(id: InvestorModel["id"]): string {
  return id === "compra" ? "cta_compra_directa_click" : `cta_${id}_partner_click`;
}

/**
 * Sección "Invertí en movilidad eléctrica" (#invertir). Contenido comercial
 * / de generación de leads: sin cifras, sin rentabilidades, sin nombres de
 * fabricantes. El detalle de cada modelo se da en una reunión, no acá.
 */
export default function InvestorModels() {
  const [activeTab, setActiveTab] = useState<InvestorModel["id"]>(
    investorModels[0].id,
  );
  const tabRefs = useRef<Record<string, HTMLButtonElement | null>>({});

  function focusTab(id: InvestorModel["id"]) {
    setActiveTab(id);
  ...
  function handleTabKeyDown(e: React.KeyboardEvent) {
    ...
    e.preventDefault();
    const ids = investorModels.map((m) => m.id);
  ...
  return (
    <section id="invertir" className="py-20 sm:py-28">
      <div className="container-evotuc">
        ...
        {/* Tarjetas: Compra directa / Fleet Partner / City Partner */}
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {investorModels.map((model, i) => {
            const theme = cardTheme[model.id];
            return (
              <Reveal as="div" key={model.id} delay={i * 0.08} className="h-full">
                <article
                  className={`flex h-full flex-col rounded-3xl p-8 shadow-soft transition-all hover:-translate-y-1 hover:shadow-lift ${theme.card}`}
                >
                  <span
                    className={`inline-flex w-fit items-center rounded-full px-3 py-1 font-mono text-[11px] uppercase tracking-wider ${theme.badge}`}
                  >
                    {model.badge}
                  </span>
                  ...
                  <p className={`mt-1 font-semibold ${theme.claim}`}>{model.claim}</p>
                  <p className={`mt-4 ${theme.copy}`}>{model.copy}</p>

                  <ul className="mt-5 space-y-2.5">
                    {model.benefits.map((b) => (
                      <li key={b} className="flex items-start gap-2.5 text-sm">
                        <CheckIcon className={`mt-0.5 h-4 w-4 flex-shrink-0 ${theme.benefitIcon}`} />
                        <span className={theme.benefitText}>{b}</span>
                      </li>
                    ))}
                  </ul>

                  <p className={`mt-5 flex-1 text-sm italic ${theme.idealFor}`}>
                    {model.idealFor}
                  </p>

                  <a
                    href={`?interes=${model.id === "compra" ? "vehiculos" : model.id}#contacto`}
                    onClick={() => trackEvent(ctaEventName(model.id))}
                    className={`mt-6 inline-flex min-h-[44px] w-full items-center justify-center rounded-2xl px-5 py-3 text-center font-semibold transition-all hover:-translate-y-0.5 ${theme.cta}`}
                  >
                    {model.ctaLabel}
                  </a>
                </article>
        ...
        <Reveal as="div" className="mt-16">
          ...
          <div
            ...
            className="mt-5 inline-flex flex-wrap rounded-2xl border border-ink/10 bg-white p-1"
          >
        ...
        {/* Comparativa rápida: Fleet Partner vs. City Partner */}
        <Reveal as="div" className="mt-16">
